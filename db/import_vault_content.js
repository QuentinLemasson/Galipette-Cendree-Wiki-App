...
const importVaultContent = async (client, directory) => {
    ...
    for (const entry of entries) {
        ...
        if (stats.isFile() && entry.endsWith('.md')) {
            ...
            try {
                await client.query(
                    'INSERT INTO notes (title, content, path, metadata) VALUES ($1, $2, $3, $4)',
                    [
                        entry.replace('.md', ''),
                        content,
...
const main = async () => {
    console.log(`Importing from ${process.env.VAULT_PATH} into ${process.env.DB_DATABASE} on ${process.env.DB_HOST}`);
    ...
    try {
        ...
        const notesDirectory = process.env.VAULT_PATH; // Using env variable for notes directory
        await importVaultContent(client, notesDirectory);
    } catch (error) {
